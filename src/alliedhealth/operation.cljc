(ns alliedhealth.operation
  "OperationActor -- one allied-health operation = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor
  (AlliedHealth-LLM) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the Allied Health Governor
  ...
  One graph run = one allied-health operation (intake -> advise ->
  ...
  decision to a human operator (licensed allied-health professional).
  The approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:actuation/administer-treatment-session` ALWAYS reaches
  this node when the governor is clean -- see `alliedhealth.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [alliedhealth.alliedhealthadvisor :as alliedhealthadvisor]
            [alliedhealth.governor :as governor]
...
(defn build
  ...
  `alliedhealth.store/Store`).
  opts:
    :advisor      -- an `alliedhealth.alliedhealthadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (alliedhealthadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; AlliedHealth-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (alliedhealthadvisor/-advise advisor store request)]
            {:proposal p :audit [(alliedhealthadvisor/trace request p)]})))

      ;; Allied Health Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
